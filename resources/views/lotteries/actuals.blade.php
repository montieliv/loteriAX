...
<style>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="saleForm" action="{{ route('lottery.sell') }}" method="POST" >
                    @csrf
    ...
    <h1>Sorteal Actual #{{ $lottery->id}}  <a href="/" class="btn btn-info mb-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                    ...
                    <div class="image-container">
                        ...
                        @php
                                    $vendida = App\Models\Venta::select('buyer_name')
                                                            ...
                                                            ->first()
                        @endphp
                        <img src="{{ asset('img/cards/' .$nameCard. '.jpg') }}" 
                            class="responsive-imagex image-container:hover responsive-image" 
                            alt="Card {{ $arrayCards[$j] }}"
                            loading="lazy">
                            <div>
                                <p class="card-text text-center mb-0">Costo {{ $arrayCostCards[$j] }}
                                Premio {{ $arrayPrizeCards[$j] }}<br>
                                <a href="#" 
                                class="btn {{ $vendida ? 'btn-danger' : 'btn-success btn-sm sell-button' }}" 
                                class="{{ $vendida ? 'deshabilitado' : '' }}"
                                data-card="{{ $arrayCards[$j] }}"
                                {{ $vendida ? 'disabled' : '' }}>
                                    {{ $vendida ?  $vendida["buyer_name"] : 'Vender' }}
                                </a>
                            </p>
                            </div>
